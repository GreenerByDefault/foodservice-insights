 ...
 *
 * Both are needed. `GetObject` reports a missing key as `NoSuchKey`,
 * while `HeadObject` reports the same thing as a bare `NotFound` with no code in the body.
 * Checking for either name alone silently misses the other operation.
 *
