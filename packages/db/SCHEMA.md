...
- **Every check, unique constraint, and trigger documented below needs a test in
  `packages/db/tests/`** asserting the database rejects the violation. That test, not this doc, is
  the durable artifact, and it's what survives the trim once migrations exist.
...
This status machine is the coordination point between the web app and the workers — its
constraints and triggers are load-bearing concurrency control, not defensive decoration. Several
exist to make the reaping race described in
[`ARCHITECTURE.md`](../../ARCHITECTURE.md#heartbeats-hangs-and-reaping) safe. Test the
queue-claiming query under concurrency too — `FOR UPDATE SKIP LOCKED` handing the same attempt to
two workers is the failure this schema exists to prevent.
...
- Check: `chart_key IS NOT NULL` if and only if `kind = 'chart'`.
...
- Primary key `(user_id, organization_id)` covers "which orgs does this user belong to".
- Index `(organization_id, user_id)` covers the reverse — an admin listing org members.
- Every org keeps an admin: a deferred trigger constraint enforcing at least one admin per
  organization, as long as the organization still exists. Deferred, so intermediate states within
  a transaction do not trip it.
