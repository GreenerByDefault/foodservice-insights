...
describe('organization', () => {
  describe('name validation', () => {
    ...
    test('rejects a name with leading or trailing whitespace', async () => {
      ...
      await expectConstraintViolation(insert, 'organization_name_trimmed');
    });
    ...
    test('rejects an empty name', async () => {
      ...
      await expectConstraintViolation(insert, 'organization_name_length');
    });
  ...
  describe('slug validation', () => {
    ...
    test.each([
      ...
      await expectConstraintViolation(insert, 'organization_slug_format');
    });
    ...
    test('rejects a slug one character over the cap', async () => {
      ...
      await expectConstraintViolation(insert, 'organization_slug_length');
    });
    ...
    test.each(RESERVED_ORGANIZATION_SLUGS)('rejects the reserved slug %s', async (slug) => {
      ...
      await expectConstraintViolation(insert, 'organization_slug_not_reserved');
    });
  });
  ...
  describe('membership invariants', () => {
    test('cannot be created with no members at all', async () => {
      ...
      await expectConstraintViolation(insert, 'organization_has_a_member');
    });
...
describe('organization_member', () => {
  describe('the at-least-one-admin rule', () => {
  ...
  describe('user deletion', () => {
    ...
    test('blocks deleting a user who is an organization’s only admin', async () => {
      // A consequence of the cascade chain auth.users -> app_user -> organization_member, and
      // the reason REQUIREMENTS.md makes an admin promote a successor before deleting their
      // account.
      const remove = withRollback(DATABASE, async (transaction) => {
        const { organization, admin } = await insertOrganization(transaction);
        const survivor = await insertAppUser(transaction);
  ...
  describe('superadmin accounting', () => {
    test('does not count a superadmin as an organization’s admin', async () => {
      // Superadmins are admins everywhere but fill no organization's admin seat, so one cannot
      // be what keeps an organization compliant.
...
describe('organization_invite', () => {
  ...
  test('rejects an expiry that precedes creation', async () => {
    ...
    await expectConstraintViolation(insert, 'organization_invite_expires_at_after_created_at');
  });
  ...
  test('allows only one pending invite per address', async () => {
    ...
    await expectConstraintViolation(
      insert,
      'organization_invite_one_pending_per_email',
      POSTGRES_CODE_UNIQUE_VIOLATION,
    );
  });
