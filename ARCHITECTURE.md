...
| Web server or worker has trouble with Supabase | Timeouts on transactions. A statement we could not complete — the database unreachable, the connection dropped, the statement cancelled — is a 503, because waiting helps. A statement Postgres *refused* is our own bug and a 500, unless a caller expected that violation and answers for itself |
