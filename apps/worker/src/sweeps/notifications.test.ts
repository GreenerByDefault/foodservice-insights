...
describe('sendPendingNotifications', () => {
  describe('which attempts it claims', () => {
    let workerId: string;
    let emailer: ReturnType<typeof recordingEmailer>;
    beforeEach(() => {
      workerId = aWorkerId();
      ...
      const outcome = await withRollback(WORKER_DATABASE, async (transaction) => {
        ...
        const emailer: Emailer = {
          ...
          transport: {
            ...
            async send(email) {
              if (email.to === broken.email) {
                throw new EmailError('the provider refused this one');
              }
  ...
  describe('sweep caps and write guards', () => {
