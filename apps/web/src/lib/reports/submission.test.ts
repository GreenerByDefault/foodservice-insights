...
import { MAX_INPUT_FILE_BYTES, MAX_MONTHS } from './limits.ts';
import {
  COUNTS_BASES,
  FIELD,
  type RawSubmission,
...
describe('validateSubmission', () => {
  test('accepts a well-formed submission', async () => {
    ...
    expect(outcome).toMatchObject({
      ...
      file: { originalFilename: 'procurement.csv', contentType: 'text/csv' },
    });
  });
  ...
  describe('rejects', () => {
    ...
    test('a file over the size cap', async () => {
      const oversized = 'x'.repeat(MAX_INPUT_FILE_BYTES + 1);

      const outcome = await validateSubmission(
        aSubmission({ file: new File([oversized], 'big.csv', { type: 'text/csv' }) }),
      );
  ...
  test('keeps the bytes of a rejected file, so the caller can store it', async () => {
    ...
    expect(outcome.file?.bytes).toEqual(new TextEncoder().encode('%PDF-1.7'));
  });

  test('has no bytes to keep when no file arrived', async () => {
  ...
  test('reports the size problem first, so a huge non-CSV is not called a non-CSV', async () => {
    ...
    const outcome = await validateSubmission(
      aSubmission({ file: new File([oversized], 'big.pdf', { type: 'application/pdf' }) }),
    );
