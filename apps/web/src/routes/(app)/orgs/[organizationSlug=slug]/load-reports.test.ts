...
function loadReports(
  ...
  args: { organizationId: OrganizationId; organizationSlug: string; cursor: ReportsCursor },
) {
  ...
  describe('which reports are returned', () => {
    test('an organization with no reports returns an empty list', async () => {
      await withRollback(database(), async (transaction) => {
        ...
        expect(data).toEqual({
          newReportHref: newReportHref(organization.slug),
          reports: [],
          olderHref: null,
          newerHref: null,
          pollHref: reportsPollHref(organization.slug),
    ...
    test("another org's reports are absent", async () => {
      await withRollback(database(), async (transaction) => {
        ...
        const data = await loadReports(transaction, {
          ...
          organizationSlug: outsider.slug,
          cursor: { direction: 'newest' },
        });
  ...
  describe('row content', () => {
    ...
    test('mints hrefs server-side', async () => {
      await withRollback(database(), async (transaction) => {
        ...
        expect(data.newReportHref).toBe(newReportHref(organization.slug));
        expect(data.reports[0]?.href).toBe(reportHref(organization.slug, report.id));
        expect(data.pollHref).toBe(reportsPollHref(organization.slug));
      });
    });
  });
