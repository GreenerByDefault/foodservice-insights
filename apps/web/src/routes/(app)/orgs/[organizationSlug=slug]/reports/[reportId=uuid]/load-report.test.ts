...
describe('a report the caller may see', () => {
  ...
  test('a report in another organization is a 404, not a leak', async () => {
    await withRollback(database(), async (transaction) => {
      ...
      await expect(statusOf(() => loadReport(transaction, outsider, report.id))).resolves.toEqual({
        status: 404,
        code: 'not_found',
      });
    });
  ...
  test('a report that does not exist is a 404', async () => {
    await withRollback(database(), async (transaction) => {
      ...
      await expect(
        statusOf(() => loadReport(transaction, organization, crypto.randomUUID() as ReportId)),
  ...
  test('a soft-deleted report is a 404, cancel request and all', async () => {
    await withRollback(database(), async (transaction) => {
      ...
      await expect(
        statusOf(() => loadReport(transaction, organization, report.id)),
      ).resolves.toEqual({ status: 404, code: 'not_found' });
    });
  });
