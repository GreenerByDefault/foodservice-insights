...
export const load: PageServerLoad = async ({ params, parent }) => {
  ...
  return await withDbErrorHandling(
    ...
    { action: 'load a report', context: { organizationId: organization.id, reportId } },
  );
};
...
type ReportRow = {
  ...
  /** The size the "Uploaded file" link's download actually serves — the workbook's, not the
   * normalized CSV's, when there was one. See `inputFile` in `ReportPageData`. */
...
export async function _loadReport(
  ...
  return {
    ...
    pollHref: reportPollHref(params.organizationSlug, row.reportId),
