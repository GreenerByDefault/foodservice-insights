 ...
 *
 * A superadmin gets a bounded query against the whole `organization` table, alphabetically.
 * Everyone else already holds their full membership list in `auth.memberships`, ordered by name
 * (see `memberOrganizations`), so this reads no database at all.
 */
